...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    var window: UIWindow?
    var current : ItemSave?

    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        current = LoadItem(filename: "current")
    ...
    func applicationWillResignActive(_ application: UIApplication) {
        if (current != nil){
            SaveItem(filename: "current", content: current!)
        }
    ...
    func LoadItem(filename : String)->ItemSave{
        let content : ItemSave = Storage.retrieve("\(filename).json", from: .documents, as: ItemSave.self)
        return content
    }
    
    func SaveItem(filename : String, content : ItemSave){
        print("saving")
